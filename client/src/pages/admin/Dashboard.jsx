    ...
    <div className="flex items-center justify-between p-4 hover:bg-(--secondary-bg) rounded-xl transition-colors cursor-pointer group border-b border-[rgb(var(--primary-accent-rgb)/0.05)] last:border-none">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-xs text-(--primary-accent) opacity-60">
            {subtitle || '—'}
          </p>
        </div>
      </div>
...
const Dashboard = () => {
  ...
  return (
    ...
    >
      ...
      {loading && (
        <div className="mb-6 sm:mb-8 text-sm text-(--primary-accent) opacity-60">Refreshing data</div>
      )}
        ...
        >
          <div className="p-6 border-b border-[rgb(var(--primary-accent-rgb)/0.05)] flex justify-between items-center">
            ...
            <button
              ...
              className="flex items-center gap-2 text-sm font-semibold text-(--primary-bg) bg-(--primary-accent) px-4 py-2 rounded-lg hover:bg-(--primary-text) transition-colors shadow-lg shadow-[rgb(var(--primary-accent-rgb)/0.2)]"
            >
              <Plus size={16} />
              Add New
            </button>
            ...
            <div className="space-y-4">
              {classrooms.slice(0, 6).map((room) => {
                ...
                return (
                  <div
                    key={room.id}
                    onClick={() => navigate(`/admin/classrooms/${room.id}`)}
