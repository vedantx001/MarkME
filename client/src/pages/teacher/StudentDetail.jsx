...
import { ArrowLeft, Calendar, UserCheck, Mail, ChevronLeft, ChevronRight } from "lucide-react";
...
const StudentDetail = () => {
  ...
  return (
    <motion.div
      ...
      className="max-w-5xl mx-auto space-y-6 sm:space-y-8 p-4 sm:p-6 lg:p-10 pb-20"
    >
      <div className="flex items-center">
        <button
          type="button"
          onClick={() => navigate(-1)}
          className="inline-flex items-center gap-2 px-4 py-2 min-h-[44px] rounded-xl bg-(--primary-bg) border border-[rgb(var(--primary-accent-rgb)/0.1)] text-(--primary-accent) font-semibold hover:bg-(--primary-accent) hover:text-white transition-colors cursor-pointer"
        >
          <ArrowLeft size={16} />
          Back
        </button>
      ...
      <div className="bg-(--primary-bg) rounded-3xl shadow-sm border border-[rgb(var(--primary-accent-rgb)/0.05)] p-5 sm:p-8 relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row gap-6 sm:gap-8 items-center md:items-start relative z-10">
          <div className="shrink-0 relative">
            <div className="w-28 h-28 sm:w-32 sm:h-32 rounded-2xl p-1 bg-linear-to-br from-(--secondary-accent) to-(--primary-bg) shadow-lg">
              <img
                src={student.photo}
                alt={student.name}
          ...
          <div className="flex-1 space-y-4 text-center md:text-left">
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-(--primary-text) tracking-tight mb-2">
                {student.name}
              </h1>

              {/* Classroom chip moved here (no more top-right floating) */}
              <div className="mb-3 flex justify-center md:justify-start">
                <span className="px-3 py-1 bg-(--secondary-bg) rounded-full text-xs sm:text-sm text-(--primary-accent) opacity-80 font-bold border border-[rgb(var(--primary-accent-rgb)/0.08)] whitespace-nowrap">
                  {classroomName}
                </span>
              </div>

              <div className="flex flex-wrap gap-2 sm:gap-3 justify-center md:justify-start text-sm text-(--primary-accent) opacity-70 font-medium">
                ...
                {student.email && (
                  <span className="flex items-center gap-1.5 px-3 py-1 bg-(--secondary-bg) rounded-full max-w-full">
                    <Mail size={14} />
              ...
              <div className="mt-3 flex justify-center md:justify-start">
                <span className="px-3 py-1 bg-(--secondary-bg) rounded-full text-xs sm:text-sm text-(--primary-accent) opacity-70 font-bold">
        ...
        {/* Divider */}
        <div className="mt-6 sm:mt-8 pt-6 sm:pt-8 border-t border-[rgb(var(--primary-accent-rgb)/0.08)] relative z-10">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
            <h3 className="text-lg sm:text-xl font-bold text-(--primary-text) flex items-center gap-2">
          ...
          {/* Scroll controls (mainly for mobile discoverability) */}
          <div className="flex items-center justify-between gap-3 mb-3">
            <button
              type="button"
              onClick={() => scrollHeatmapBy(-280)}
              className="inline-flex items-center justify-center w-11 h-11 rounded-xl bg-(--secondary-bg) border border-[rgb(var(--primary-accent-rgb)/0.08)] text-(--primary-accent) hover:bg-(--primary-accent) hover:text-(--primary-bg) transition-colors"
              aria-label="Scroll attendance history left"
            >
              <ChevronLeft size={20} />
            </button>

            <div className="text-xs font-semibold text-(--primary-accent) opacity-60 text-center">
              Swipe or use arrows
            </div>
            ...
            <button
              ...
              onClick={() => scrollHeatmapBy(280)}
              className="inline-flex items-center justify-center w-11 h-11 rounded-xl bg-(--secondary-bg) border border-[rgb(var(--primary-accent-rgb)/0.08)] text-(--primary-accent) hover:bg-(--primary-accent) hover:text-(--primary-bg) transition-colors"
              aria-label="Scroll attendance history right"
            >
          ...
          <div className="relative">
            {/* subtle gradient cues on small screens */}
            <div className="pointer-events-none absolute inset-y-0 left-0 w-10 bg-linear-to-r from-(--primary-bg) to-transparent md:hidden" />
            <div className="pointer-events-none absolute inset-y-0 right-0 w-10 bg-linear-to-l from-(--primary-bg) to-transparent md:hidden" />

            <div
              ref={heatmapScrollRef}
              className="flex gap-8 overflow-x-auto pb-6 scrollbar-thin scrollbar-thumb-(--primary-accent)/10 hover:scrollbar-thumb-(--primary-accent)/20"
            >
                  ...
                  <div className="grid grid-cols-7 gap-1.5">
                    ...
                    {month.days.map((day) => {
                      ...
                      return (
                        ...
                        >
                          {day.status ? day.dayNumber : (
                            <span className="text-(--primary-accent) opacity-30 font-normal">{day.dayNumber}</span>
                          )}
                        </motion.div>
                      );
                    })}
