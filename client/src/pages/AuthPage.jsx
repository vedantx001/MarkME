...
const AuthPage = () => {
  ...
  // State for the sliding toggle animation
  const [toggleIndex, setToggleIndex] = useState(0);
  const roles = ['Admin', 'Teacher', 'Principal'];

  useEffect(() => {
    // Update the index whenever the role changes for the animation
    const index = roles.findIndex(r => r.toLowerCase() === role);
  ...
  const colors = {
    ...
    skyBlue: '#85C7F2',
    onyx: '#0E0E11',
    // Added a slightly darker, richer blue for the main button action
    primaryDark: '#3182CE', 
  };
  ...
  // Reusable Feature Card Component for the Left Panel
  const FeatureCard = ({ icon: Icon, title, description }) => (
    <div className="bg-white/10 backdrop-blur-md p-5 rounded-2xl border border-white/10 hover:bg-white/20 transition-all duration-300 group">
      <div className="bg-white/20 w-10 h-10 flex items-center justify-center rounded-lg mb-3 group-hover:scale-110 transition-transform">
        <Icon size={20} style={{ color: colors.skyBlue }} />
      </div>
  ...
  return (
    ...
    >
      <div className="w-full max-w-[1200px] h-full md:h-[850px] bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row">
        ...
        >
          ...
          <div className="absolute bottom-0 right-0 w-[500px] h-[500px] bg-[#85C7F2] opacity-10 rounded-full translate-x-1/3 translate-y-1/3 blur-3xl z-0"></div>

          {/* Header Section */}
          <div className="relative z-10">
            <h1 className="text-4xl font-extrabold tracking-wider flex items-center gap-3">
              <ShieldCheck className="w-10 h-10" style={{ color: colors.skyBlue }} />
              MARK<span style={{ color: colors.skyBlue }}>ME</span>
            </h1>
          ...
          <div className="relative z-10 grid grid-cols-2 gap-4 mt-4 flex-grow content-start">
             <FeatureCard 
                icon={Workflow}
        ...
        <div className="w-full md:w-7/12 p-8 md:p-16 flex flex-col justify-center bg-[#FBFDFF]">
          ...
          <div className="max-w-md mx-auto w-full">
            ...
            <div className="mb-10 text-center md:text-left">
              <h2 className="text-3xl md:text-4xl font-bold mb-3" style={{ color: colors.onyx }}>
              ...
              <div className="relative flex bg-gray-100 p-1 rounded-xl mb-8">
                ...
                <div 
                  ...
                  style={{ 
                    ...
                    left: `calc(${toggleIndex * (100 / roles.length)}% + 4px)` 
                  }}
                ></div>
                ...
                {roles.map((r, index) => (
                  <button
                    ...
                    className={`flex-1 py-3 text-sm font-bold rounded-lg relative z-10 transition-colors duration-300 ${
                      toggleIndex === index
                        ? 'text-[#2D3748]' 
...
// Check your tailwind.config.js to ensure custom classes work, or add this small CSS snippet
const customStyles = `
  .form-input {
    @apply w-full pl-14 pr-4 py-4 rounded-xl border-2 border-gray-100 bg-gray-50/50 text-gray-800 font-medium focus:outline-none focus:border-[#3182CE] focus:bg-white focus:ring-4 focus:ring-[#3182CE]/10 transition-all duration-300;
  }
`;
